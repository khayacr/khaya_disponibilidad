...
- Payment dates for reservas/cuotas use `frontend/src/utils/paymentCalendar.js`: same **calendar day** each month across **different months** (first cuota = month after reserva); dates are normalized at local noon and `YYYY-MM-DD` strings are parsed as **local** civil dates so UTC does not shift the day (`UnitModal.jsx` / `generateReservationPDF.js`).
